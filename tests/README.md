...
- strict schema-2 checkpoint progress, schema-1 migration, corrupt/oversized/denied fallback, persistent genuine unlocks and per-stage loadouts, debug-write rejection, locked-card guards, and fresh Sector 1 Continue starts that restore only the selected weapon checkpoint;
- accessible New Game overwrite confirmation with safe cancellation and retained local record/preferences, plus non-destructive Restart and Play Again paths;
...
- point-only normal stars, transition-only streaks, continuous Earth-to-deep-space scenery, local authored raster worlds for Stages 4–9, deterministic stage previews, exactly three progressive asteroid crack thresholds plus hit flash, and bounded game-over shake/flash that cannot leak into menus;
...
- strict CSP, local repository-subpath resources, no network/dynamic code/dependencies, JavaScript syntax, symlinks, payload limits, complete local-asset reference coverage, no unused runtime rasters, and referenced size-bounded README WebP captures;
