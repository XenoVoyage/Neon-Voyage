...
| `visuals.test.js` | Shell-owned renderer sizing, pointer-only reticle, capped clustered off-screen objective cues, twenty-scene continuity, exact complete gameplay-raster mapping and draw coverage, cached late/boss washes, deterministic Enigma micro-previews, clear-versus-travel hyperspace contracts, projectile/field contracts, damage presentation, and compact responsive summary/dialog rules |
...
| Inner Belt surge | Play Stage 2 from its Stage 1 loadout; confirm the six-root opening stays readable, reserve batches prevent empty downtime without flooding split fragments, the Colossal arrives after the opening formation with a clear cue, and the complete encounter feels close to the three-minute target rather than repetitive or grindy |
...
| Campaign and bosses | Complete Stage 1–20; confirm staged asteroid and alien novelty, balanced mixed groups, bounded massive-root pressure, all stages using the expanded finite field and bounded camera without a boss ring, node-dependent Leviathan reflection, touch auto-aim preferring live nodes/other threats over each damage-reduced command-ship body, clean escort requirements, and bounded next-sector wrap |
