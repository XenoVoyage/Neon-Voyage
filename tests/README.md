...
| `visuals.test.js` | Shell-owned renderer sizing, pointer-only reticle, capped clustered off-screen objective cues, terminal ship suppression, opt-in shake/flash rendering, seven-scene continuity, exact gameplay-raster mapping/draw coverage, raster-led overlay contracts including stripe-free Overdrive art, deterministic irregular asteroid fractures, semantic pickup identities, hull damage states, cached late/boss washes, Enigma micro-previews, cinematic contracts, and compact responsive rules |
