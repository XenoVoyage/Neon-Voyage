...
Observed for Neon Voyage 1.2.3 on 2026-08-13: **81/81 tests passed** with Node v24.14.0 on Linux x64. The focused mobile input suite passes **29/29** checks.
...
- hybrid tablet touch detection, stale mouse-target handoff, real simultaneous move/aim pointer ownership, dynamic origin placement across each playable canvas half, neutral initial contact, radial response, magnitude-scaled movement and turning, the 7.2-radian-per-second full-aim cap, shared barely-active aim ownership, low-band aim/fire alignment, heading preservation, aim-to-fire, and independent Dash and Pulse actions;
- global release/cancel cleanup, lost-capture and rejected-capture recovery, stale-fire ownership guards, mobile browser-focus tolerance with hidden-document, page-exit, and desktop focus pause retained, stationary capture-safe touch pause/resume, plus full portrait ownership of covered UI/keyboard/gamepad input, held-button edge suppression, seamless landscape resume, safe-area and non-overlap contracts, accessible double-tap suppression, and harmless missing or rejected orientation locks;
