...
module.exports = function register(test) {
  ...
  test("runtime metadata and public documentation agree on version v2026.8.21", () => {
    const version = readProject("VERSION.txt").trim();
    assert.equal(version, "Neon Voyage v2026.8.21");
    assert.match(readProject("js/config.js"), /version:\s*["']v2026\.8\.21["']/);
    assert.match(readProject("index.html"), />Version v2026\.8\.21</);
    assert.match(readProject("README.md"), /Version v2026\.8\.21/);
    assert.match(readProject("CHANGELOG.md"), /^## \[v2026\.8\.21\] — 2026-08-21$/m);
    assert.match(readProject("AUDIT.md"), /^# Neon Voyage v2026\.8\.21/m);
