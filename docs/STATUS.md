...
| Current runtime version | `v2026.8.20` in the source checkpoint; Pages follows successful `main` deployments |
...
`v2026.8.20` is the current runtime label. A local label, changelog heading, Pages deployment, or version badge does not by itself prove that a matching immutable tag and formal GitHub Release exist; publication status must be confirmed from GitHub.
