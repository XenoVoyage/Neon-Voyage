...
| Current runtime version | `v2026.8.21a` in the source checkpoint; Pages follows successful `main` deployments |
...
| Product work | Seven-stage pacing, early Titan and alien contact, frequent longer rewards, asteroid-only Pulse attraction, crystal shrapnel, semantic pickups, damage-state effects, expanded finite scrolling fields, raster-led gameplay effects, and louder adjustable synthesized audio are implemented |
...
`v2026.8.21a` is the current runtime label. A local label, changelog heading, Pages deployment, or version badge does not by itself prove that a matching immutable tag and formal GitHub Release exist; publication status must be confirmed from GitHub.
