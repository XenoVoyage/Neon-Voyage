...
| Runtime version | `v2026.8.21e`; `js/config.js` is canonical and intentional mirrors are consistency-tested |
...
- No immutable Git tag or GitHub Release exists for `v2026.8.21e`. Creating one remains an explicit owner publication decision and is not implied by a changelog heading or Pages deployment.
