...
| Area | Verified state |
| --- | --- |
| Product | Complete twenty-stage offline browser campaign with Harrower, Leviathan, and Sovereign command-ship encounters |
| Runtime version | `v2026.8.21f`; `js/config.js` is canonical and intentional mirrors are consistency-tested |
| Runtime surface | Repository-local HTML, CSS, five classic scripts, Canvas, Web Audio, and 58 WebP runtime rasters |
...
| Saved data | Unchanged local keys; compatible preferences with default-off shake/flash fields, plus strict schema-5 checkpoints with exact schema-4/schema-3/schema-2/schema-1 migration |
...
- Automated coverage exercises offline boot, strict CSP and local resources, all supported input families, responsive simulated viewports, accessibility and focus, saved-progress migrations, the complete Stage 1–20 campaign, all three bosses, renderer ownership, audio bounds, and deterministic long-run caps.
...
- No physical phone or iPad session, subjective audio listening, or uninterrupted human-controlled twenty-stage completion is currently claimed. Those are product-feel observations, not hidden source-gate results.
- No immutable Git tag or GitHub Release exists for `v2026.8.21f`. Creating one remains an explicit owner publication decision and is not implied by a changelog heading or Pages deployment.
