...
| `neon-voyage-progress-v1` | Schema-5 unlocked stages, twenty bounded checkpoints, 13 Mk V module tiers, and up to four base durations for each of eight saved temporary effects | Preserve tested exact-shape migrations from schema 4, schema 3, schema 2, and schema 1 |

The storage key remains unchanged. Schema 5 accepts only the exact current module and timer keys, Stage 1–20 bounds, Mk I–V tiers, four-duration timer ceilings, and a 16,384-byte record limit. A valid schema-4 record retains its exact historical 13-module/eight-timer shape and expands the former seven-stage checkpoints through the fixed endpoints `1→1`, `2→5`, `3→6`, `4→9`, `5→10`, `6→18`, and `7→20`; each old loadout fills the newly inserted stages up to its endpoint, preserving earned strength without unlocking beyond equivalent narrative progress. A valid schema-3 record retains its historical 13-module/seven-timer shape and Stage 1–20 bounds, migrates one-to-one, and receives a safe zero `thrusterTimer`. Valid schema-2 records retain their exact historical seven-module/five-timer shape and Stage 1–9 bounds during one-to-one migration; valid schema-1 unlock progress migrates separately. Malformed, unknown, oversized, or out-of-range records are rejected rather than partially trusted. Live hull, score, position, clocks, cooldown phase, entities, generated Enigma cards, and paused battles are never saved as campaign checkpoints.
...
| --- | --- |
| Save compatibility | `validSave`, strict schema-5 checkpoint/progress validators, schema-4/schema-3/schema-2/schema-1 migration, `saveLocal`, `saveProgress` |
...
- `ND.game` exposes the intentional deterministic simulation surface used by tests, including all three configured bosses, the Stage 1–20 journey, passive-system state, the Enigma snapshot, and controlled enhancement selection.
